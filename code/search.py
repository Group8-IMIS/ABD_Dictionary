...
class Dictionary:
    ...
    def translate(self, unknown_word):
        ...
        if translation:
            ...
            # 如果没有找到精确匹配，尝试模糊匹配
            close_matches = difflib.get_close_matches(word_to_lookup, [entry['word'] for entry in self.dictionary], n=3, cutoff=0.6)

            if close_matches:
                suggestions = "\nDid you mean one of these?\n" + "\n".join(close_matches)
                return f"{unknown_word} not found. {suggestions}"
            else:
